...
Teraz rozpocznie się zadanie wymagające oceny i zapamiętywania słów.
Na ekranie komputera będą się pojawiały, jedno po drugim, różne słowa.
Każde słowo będzie wyświetlane przez kilka sekund.
...
d = read.csv('slowa.csv', sep = '@')
cnd = db.random.condition(c('same', 'diff'))
if(cnd == 'diff'){
    word.gender = c(M = 'female', K = 'male')[USER.DATA$gender]
}else{
    word.gender = c(M = 'male', K = 'female')[USER.DATA$gender]
}
run.trials(trial.code, expand.grid(scale = 'emotion', samegender = cnd,
                                   word = c(sample(as.character(d[[word.gender]])))),
           record.session = T,
           condition = 'default')
